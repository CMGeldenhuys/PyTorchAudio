...
def _crop_audio_label(
    ...
    assert kernel_size.is_integer(), f"kernel_size={kernel_size} must be interger"
    assert stride.is_integer(), f"stride={stride} must be interger"
    ...
    label_offset = max(math.floor((frame_offset - kernel_size) / (stride)) + 1, 0)
    num_label = math.floor((num_frames - kernel_size) / (stride)) + 1
